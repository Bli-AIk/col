...
impl<'ctx> JITExecutor<'ctx> {
    ...
    pub fn execute_main(&self) -> Result<f64, String> {
        unsafe {
            let main_fn: JitFunction<unsafe extern "C" fn() -> f64> = self.execution_engine
                .get_function("main")
    ...
    pub fn execute_function(&self, name: &str, args: &[f64]) -> Result<f64, String> {
        match args.len() {
            0 => {
                unsafe {
                    let func: JitFunction<unsafe extern "C" fn() -> f64> = self.execution_engine
                        .get_function(name)
            ...
            1 => {
                unsafe {
                    let func: JitFunction<unsafe extern "C" fn(f64) -> f64> = self.execution_engine
                        .get_function(name)
                        .map_err(|e| format!("Failed to get function '{}': {}", name, e))?;
            ...
            2 => {
                unsafe {
                    let func: JitFunction<unsafe extern "C" fn(f64, f64) -> f64> = self.execution_engine
                        .get_function(name)
                        .map_err(|e| format!("Failed to get function '{}': {}", name, e))?;
